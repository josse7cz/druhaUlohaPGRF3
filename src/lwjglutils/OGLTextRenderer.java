...
public class OGLTextRenderer {
	...
	private static class Viewer {
		...
		private void view(int textureID, double x, double y, double w, double h) {
			if (glIsProgram(shaderProgram)) {
				...
				glUniformMatrix4fv(locMat, false, ToFloatArray
					.convert(new Mat4Scale(w, h, 1).mul(new Mat4Transl(x,y,0))));
	...
	public void addStr2D(int x, int y, String s) {
		if (s != null){
			...
			if (w>0 && h>0) {
				...
				viewer.view(textureID, x/(double)width, 1-y/(double)height,w/(double)width, h/(double)height);
			}
		}
	}
